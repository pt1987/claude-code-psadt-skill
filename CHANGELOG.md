...
five-scenario sandbox gate. It was still wrong. Every Chrome build ships a **new MSI ProductCode**
(153: `{CB47EDA1-...}`, 154: `{C8B3A0BF-...}`), and GoogleUpdater replaces the binaries in place without
re-running the MSI. `New-MsiPackage.ps1` keyed the detection script, the Uninstall hook and the Repair hook
on this build's GUID, so one update later:

| Keyed on the ProductCode | On a device the updater already moved on |
|---|---|
| Detection | the next package never matches -> "not installed" -> Intune reinstalls every ~24h |
| Install | older MSI over a newer build -> 1603 (downgrade) |
| Uninstall / Repair | aimed at a GUID the device does not have |

The sandbox cannot see this: it proves the package on a machine where the app has never updated itself.
The EXE generator had drawn this conclusion long ago (version floor); the MSI generator had not.

**`New-MsiPackage.ps1 -SelfUpdatingBinary '<Vendor>\<App>\<app>.exe'`** (relative to Program Files;
`-ArpDisplayName` defaults to `-AppName`) now generates:

- a detection script with a **version floor** on the binary, Program Files resolved via `ProgramW6432`
  because Intune may run it 32-bit;
- an Install hook that **returns early on an equal or newer build**, before closing any process, via
  `Get-InstalledBinaryVersion` in the Extensions module;
- Uninstall via `Uninstall-ADTApplication -Name '<ARP name>' -NameMatch Exact -ApplicationType MSI`, and
  Repair on whichever MSI `Get-ADTApplication` finds registered (packaged MSI if none);
- `package.detection = versionFloor` and `package.selfUpdating` in the manifest.

Without the parameter the output is **byte-identical** to 0.43.0 (launcher, detection script and
Extensions module generated by both, 0 differing lines). Verified on Chrome 154.0.8037.58: full sandbox
gate 20/20 assertions, detection negative in 32- and 64-bit PowerShell on a host running Chrome 153. New
functional tests run the generator in both modes (`tests/New-MsiPackage.Tests.ps1`), including an AppName
with `'` and `$` staying inert in the generated detection and hooks - the operator-value class 0.43.0
closed for the default mode, held for the new one too.

### Every other learning of that run, as a gate rather than a sentence

The run took ~35 minutes for ~10 minutes of packaging. None of the causes was PSADT; each was a rule the
operator was trusted to remember although a script could check it.

- **Research before scaffold is enforced.** `Get-PsadtLocalEvidence.ps1` records the questions it sends
  to researchers per installer SHA256 (`<config home>\evidence\<sha256>.json`). Pre-flight check 11,
  `Research`, is FAIL until each has `research.answers.<id>` in the manifest (WARN if the ladder never ran
  for that installer). On Chrome 154 a full sandbox gate ran while the pitfall researcher was still out,
  and its answer rewrote every hook.
- **The ladder asks the question that was missing:** `self-updating` - does the app update itself, and
  does every build ship a new ProductCode? It shares the `intune` family with the pitfalls, so the agent
  budget does not grow, and an earlier package of the same product with a different ProductCode is
  handed to that agent as KnownContext. Accepted unanswered for non-MSI (the EXE generator already
  detects by a version floor).
- **The Reviewer-GREEN handoff is code.** New `scripts/Test-PsadtPreflightCurrent.ps1`:
  `Invoke-PsadtPackage.ps1` and `Invoke-PsadtSandboxTest.ps1` refuse to run unless `results.preflight` is
  GREEN and newer than the launcher, Extensions module, detection script and `Files\`/`SupportFiles\`.
  `-SkipPreflightGate` is the deliberate bypass (fixtures, forensics); `-GenerateOnly` is not gated.
- **The sandbox names its live progress file first** (`Live progress (host path ...)`); phase 6.1 says
  to poll it rather than pipe the output, which stays empty until the end and read as a hung VM.
- **The upload takes the rest of what the manifest recorded** (0.43.0 already took the `.intunewin`):
  `-LogoPath` defaults to `artifacts.logo`, `-Description` to `app.description.<language.dossier>`; a
  `versionFloor` package uses `artifacts.detection` and **refuses** `-MsiProductCode`.
- **The dossier reads what the pipeline produced:** `app.description.de/en` (one text for dossier and
  upload), `results.preflight.checks` (pre-flight now records its checks; the dossier said "not run" on
  a GREEN package), and each hook's `##` rationale comments from the launcher, in source order.
- **Fix the tool before the package.** Phase 4.3 and SKILL.md Phase 3: a gap no generator covers goes
  into the generator first, then generate once - the Chrome run gated a hand-patched package and then the
  regenerated one.

Tests: `tests/ChromeLearnings.Tests.ps1` (24) holds every item above as behaviour;
`tests/New-MsiPackage.Tests.ps1` runs the generator in both modes. SKILL.md stays inside the
compaction budget (Phase 0-6 < 17500 bytes).

Docs: phase 1.3 (answers + `self-updating`), 4.3 (when and how), 5 (the enforced verdict, check 11),
6.1 (live progress), 9 (manifest-driven upload), App. G (the incident and the cost table), SKILL.md
Phases 2, 3, 5, 6, 8.
