...
param(
    ...
    # now removes it: wintrust asked the disabled Defender to rate each signed package and waited ~2
    # minutes per file. Anything still slow after that fix is genuine installer work. Defender scans every file it
    # sees, so an install that takes 20 seconds on real hardware can take two minutes here.
    # 600 s, not 900. A silent install that has not returned in ten minutes is almost never "still
    # working" - it is waiting for something that will never arrive, and every extra minute of timeout
    # is a minute an operator stares at a VM before being told so. Raise it deliberately for the rare
...
# Every deployment action below runs as SYSTEM through a scheduled task, and a SYSTEM task draws NOTHING
# on the interactive desktop. The runner's own console was meant to be what an operator watches, but it
# cannot be relied on: on some Windows Sandbox builds the <LogonCommand> process gets no visible console
# window at all. Measured 2026-09-14 while packaging Citrix Workspace - ten minutes into a healthy
# install the guest showed wallpaper and nothing else, and Task Manager listed no windowed PowerShell.
# So: try to show this console AND record whether one even exists, then start a separate top-most
# window that does not depend on the answer.
...
function Disable-GuestSmartAppControl {
    <#
      THE reason heavy packages "cannot be tested in the sandbox" - and it is a sandbox bug, not a
      package one. Microsoft's own root cause (Windows-Sandbox issue #85): Smart App Control is ENABLED
      in the base image while Windows Defender is DISABLED. wintrust asks windefend for the trust level
      of every signed package, windefend never answers, and the client DLL sits in a retry loop. The MSI
      log signature is a ~2 minute stall at "SOFTWARE RESTRICTION POLICY: Verifying package" for EACH
      package. A bootstrapper chaining a dozen MSIs therefore takes half an hour and looks like a hang -
      measured here three times on Citrix Workspace, and recorded earlier against the ADK.
      Reported effect of turning SAC off: .NET runtime 6 min -> 5 s, VC++ redist 4:04 -> 0:04.
      Safe here by construction: the VM is discarded when the run ends.
    #>
    $before = Get-Date
    $state = 'not-applied'
    try {
        ...
        if (Test-Path -LiteralPath $key) {
            Set-ItemProperty -LiteralPath $key -Name 'VerifiedAndReputablePolicyState' -Value 0 -Type DWord -ErrorAction Stop
            $state = 'registry set'
            $ciTool = Join-Path $env:SystemRoot 'System32\CiTool.exe'
            if (Test-Path -LiteralPath $ciTool) {
                & { $ErrorActionPreference = 'Continue'; & $ciTool -r 2>&1 | Out-Null }
                $state = 'registry set + policies refreshed'
            }
        }
        else { $state = 'CI policy key absent' }
    }
    catch { $state = "failed: $($_.Exception.Message)" }
    return [pscustomobject]@{ State = $state; Seconds = [int]((Get-Date) - $before).TotalSeconds }
}

function Get-GuestLogTail {
    <#
      "still running - 883s" says the harness is alive and nothing about the INSTALL. During a 15-minute
      vendor install that is the difference between an informed wait and a blind one, so the newest PSADT
      session log is tailed and its last meaningful line travels with the heartbeat. Best effort by
      design: no log yet, a locked file or an odd format must never interrupt a healthy run.
    #>
    try {
        $log = Get-ChildItem 'C:\Windows\Logs\Software\*.log' -ErrorAction SilentlyContinue |
            Sort-Object LastWriteTime -Descending | Select-Object -First 1
        if (-not $log) { return '' }
        $line = Get-Content -LiteralPath $log.FullName -Tail 15 -ErrorAction SilentlyContinue |
...
function Save-TimeoutDiagnostics {
    <#
      A timed-out action is the one case where the harness knows the LEAST and needs to know the most.
      PSADT's log cannot explain it - PSADT is the thing still waiting - and the installer's own log
      stays inside the VM, which is discarded seconds later. Twice on 2026-09-14 that left nothing but
      "awaiting completion" and a guess.

      So on a timeout, and only then, two things are captured before the guest shuts down:
        * the full process table WITH command lines - which shows whether the installer spawned a child
          that is itself waiting, and on what;
        * every vendor log touched in the last two hours, which is where an installer that refuses to
          continue says why.
      Bounded by age, size and count, because this runs while the operator is already waiting.
    #>
    param([string]$Label)
    try {
        ...
        $copied = 0
        foreach ($root in @($env:ProgramFiles, ${env:ProgramFiles(x86)}, $env:ProgramData, $env:TEMP, (Join-Path $env:WinDir 'Temp'))) {
            if (-not $root -or -not (Test-Path -LiteralPath $root)) { continue }
            $logDirs = Get-ChildItem -LiteralPath $root -Directory -Recurse -Depth 3 -Filter '*Log*' -ErrorAction SilentlyContinue
            foreach ($ld in $logDirs) {
                if ($copied -ge 60) { break }
                Get-ChildItem -LiteralPath $ld.FullName -File -Recurse -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -gt $cutoff -and $_.Length -lt 5242880 } |
                    Select-Object -First 20 |
                    ForEach-Object {
                        $target = Join-Path $dir ('vendor_' + $ld.Name + '_' + $_.Name)
                        Copy-Item -LiteralPath $_.FullName -Destination $target -Force -ErrorAction SilentlyContinue
                        $script:copied = $copied++
                    }
            }
        }
        Write-Host ("   !! timeout diagnostics captured: {0} (process table + vendor logs)" -f $dir) -ForegroundColor Yellow
    }
    catch { }
}
...
$consoleWindow = $false
try {
    Add-Type -Namespace PsadtSandbox -Name Win -MemberDefinition @"
...
$report.consoleWindow = $consoleWindow

Set-Progress -Step 'starting' -State 'preparing the guest'

$progressUi = Join-Path $mapped '_SandboxProgressUi.ps1'
if (Test-Path -LiteralPath $progressUi) {
    try {
        # NO -WindowStyle here. Start-Process passes the style into the child's STARTUPINFO, and the
        # FIRST window the process creates inherits it - so -WindowStyle Hidden made the WinForms form
        # itself invisible while the process ran happily in the background. Measured 2026-09-14: hidden
        # gives a live process and no window, plain gives a live process and a window. The console this
        # opens is a second piece of visible feedback, which is the opposite of a problem here.
        Start-Process -FilePath 'powershell.exe' -ArgumentList @(
            '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', ('"' + $progressUi + '"'),
...
function Invoke-AsSystem {
    ...
    # schtasks writes its "/ST is earlier than current time" notice to stderr on EVERY create, because
    # this design provokes it deliberately (the task is started by /Run, never by its schedule).
    # Redirecting with 2>file still lets the transcript record the ErrorRecord, so an operator watching
    # the guest sees a red NativeCommandError block with a call site on a run that is perfectly healthy -
    ...
    & {
        ...
        ($runOutput | Out-String) | Set-Content -LiteralPath $schtasksErr -Encoding UTF8 -ErrorAction SilentlyContinue
    }
    ...
    while ((Get-Date) -lt $deadline) {
        ...
        if (Test-Path -LiteralPath (Join-Path $mapped 'STOP.txt')) {
    ...
    return [pscustomobject]@{ ExitCode = [int]$raw.Trim(); Output = $out; TimedOut = $false; Seconds = [int]((Get-Date) - $startedAt).TotalSeconds }
}
...
try {
    ...
    try {
        ...
        # NOT -ErrorAction Stop: a terminating error is written to the transcript even when it is caught,
        # so the healthy "WMI is broken, repair it" path printed a red TerminatingError block before the
        # repair line that fixes it. Probing softly and deciding on the RESULT keeps the transcript
        # honest about what actually went wrong.
        $probe = Get-CimInstance -ClassName Win32_ComputerSystem -ErrorAction SilentlyContinue
        if (-not $probe) { throw 'Win32_ComputerSystem returned nothing (access denied or repository damaged)' }
        $wmiState = 'OK'
    } catch {
    ...
    # (Citrix Workspace, 2026-09-14) where staging was the largest fixed cost of the whole run:
    #   * Copy-Item walks the tree single-threaded across the sandbox's mapped-folder transport.
    #     robocopy /MT moves the same tree in parallel, ships with every Windows image, and reports
    #     anything below exit 8 as success. Its stderr is trapped the same way the schtasks calls are:
    #     $ErrorActionPreference is 'Stop', and in WinPS 5.1 a native command writing to stderr raises a
    #     terminating NativeCommandError that would end the run during STAGING.
    #   * Unblock-File was called on EVERY file. A PSADT package carries hundreds of localized string
    #     files and assets that no loader ever refuses for a zone mark; only scripts, modules and
    #     binaries are worth the per-file stream operation.
    $stageStart = Get-Date
    & {
        $ErrorActionPreference = 'Continue'
    ...
    $report.verdict = if ($report.failedAssertions.Count -ne 0) { 'RED' }
    elseif ($report.fullGate) { 'GREEN' }
    else { 'GREEN_PARTIAL' }
}
catch {
    if ($script:cancelRequested) {
        $report.verdict = 'STOPPED'
        $report.error = 'Cancelled from the host (STOP.txt). The guest shut itself down, so the work folder is released and the next run is not blocked.'
        Write-Host 'CANCELLED from the host - shutting the guest down cleanly.' -ForegroundColor Yellow
    }
    else {
    ...
    Set-Progress -Step 'finished' -State 'finished' -Verdict $report.verdict
...
$runner = $runnerTemplate.
    ...
    Replace('__SCENARIOS__', (ConvertTo-PsArrayLiteral $Scenarios))
...
# The guest-side progress window travels with the runner. It runs as its own process inside the VM, so
# it still shows the run's state when the <LogonCommand> console has no window - which is what turned a
# healthy 15-minute Citrix install into a blank screen on 2026-09-14.
...
if (Test-Path -LiteralPath $progressUiSource) {
    ...
    $showCmd = "@echo off`r`npowershell.exe -NoProfile -ExecutionPolicy Bypass -File `"%~dp0_SandboxProgressUi.ps1`" -ProgressFile `"%~dp0results\progress.json`" -TranscriptFile `"%~dp0results\sandbox-transcript.txt`"`r`n"
    [System.IO.File]::WriteAllText((Join-Path $workFolder 'SHOW-PROGRESS.cmd'), $showCmd, (New-Object System.Text.ASCIIEncoding))
}
        ...
        try {
            ...
            if ($p) {
                ...
                if ($line -ne $lastShown) {
                    $lastShown = $line
                    Write-Host ('   guest: ' + $line) -ForegroundColor DarkGray
                }
            }
        }
        catch { }
...
if (-not $KeepSandboxOpen) {
    if ($hostTimedOut) {
        # Ask the GUEST to stop. Killing the VM from here would orphan the vmmemWindowsSandbox worker,
        # which then holds the work folder until a reboot and blocks the next run - so the host drops a
        # STOP.txt that the runner polls, and the guest shuts ITSELF down, which is the only clean
        # teardown. Before this existed the host simply gave up and left a VM running behind it.
        Write-Warning "The host stopped waiting after $TotalTimeoutMinutes minutes. Asking the guest to stop cleanly (STOP.txt); raise -TotalTimeoutMinutes if the package simply needs longer."
        try { Set-Content -LiteralPath (Join-Path $workFolder 'STOP.txt') -Value 'host timeout' -Encoding ASCII } catch { }
        $stopDeadline = (Get-Date).AddSeconds(120)
        while ((Get-Date) -lt $stopDeadline) {
            if (Test-Path -LiteralPath $donePath) { break }
            if (-not (@(Get-Process -Name 'WindowsSandboxServer' -ErrorAction SilentlyContinue).Count -gt 0)) { break }
            Start-Sleep -Seconds 5
        }
        if (-not (Test-Path -LiteralPath $donePath)) {
            Write-Warning "The guest did not acknowledge the stop within 120 seconds. Close the Windows Sandbox window yourself and confirm the discard prompt - that releases '$workRoot'."
        }
    }
...
if (-not $result) {
    $reason = if (-not $sawSandbox) { "Windows Sandbox never started - check that no policy blocks it." }
              elseif (-not (Test-Path -LiteralPath (Join-Path $resultsFolder 'sandbox-transcript.txt'))) {
                  # The VM came up and went away again without the runner ever writing a line. That is a
                  # FAILED START, not an interrupted test, and saying "the VM was closed" for it sends the
                  # operator looking for a person who closed a window. Measured 2026-09-14: a sandbox
                  # launched moments after the previous one shut down died after ~40 seconds with an empty
                  # transcript, and the harness reported it as if someone had cancelled.
                  "Windows Sandbox started but the runner never produced any output - the VM died during boot. This is a SANDBOX fault, not a package fault. It is most often a sandbox started while the previous one was still tearing down; wait for vmmemWindowsSandbox to exit and run again."
              }
