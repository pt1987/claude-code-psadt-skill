...
Describe 'New-WindowsFeaturePackage: the template Replace chain is unbroken' {
...
Describe 'New-WindowsFeaturePackage.ps1 refuses the comment terminator in Author and Changelog (0.43.0)' {
    ...
    BeforeAll { . "$PSScriptRoot/_helpers.ps1"; $script:genSrc = Join-Path $PSScriptRoot '..\scripts\New-WindowsFeaturePackage.ps1'; $script:genText = Get-Content -LiteralPath $script:genSrc -Raw }
